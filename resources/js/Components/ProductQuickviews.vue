<template>
    <TransitionRoot as="template" :show="show||false">
                                                ...
                                                <div class="flex items-center">
                                                    <div class="flex items-center">
                                                        <StarIcon v-for="rating in [0, 1, 2, 3, 4]" :key="rating"
                                                                  :class="[product_.rating > rating ? 'text-gray-900' : 'text-gray-200', 'h-5 w-5 flex-shrink-0']"
                                                                  aria-hidden="true"/>
                                                    </div>
                                                    <p class="sr-only">{{ product_.rating }} out of 5 stars</p>
                                                    <a href="#"
                                                       class="ml-3 text-sm font-medium text-indigo-600 hover:text-indigo-500">{{
                                                            product_.reviewCount
                                                        }} reviews</a>
                                                </div>
                                            </div>
                                        </section>
                                            ...
                                            <form>
                                                ...
                                                <div>
                                                    ...
                                                    <RadioGroup v-model="selectedColor" class="mt-4">
                                                        <RadioGroupLabel class="sr-only"> Choose a color
                                                        </RadioGroupLabel>
                                                        <span class="flex items-center space-x-3">
                              <RadioGroupOption as="template" v-for="color in product_.colors" :key="color.name"
                                    ...
                                    <svg class="absolute inset-0 h-full w-full stroke-2 text-gray-200"
                                      ...
                                      <line x1="0" y1="100" x2="100" y2="0" vector-effect="non-scaling-stroke"/>
                                                ...
                                                <button type="button"
                                                        ...
                                                        @click="AddToCart(product)">Add to bag
                                                </button>
                                            </form>
                                        </section>
                                    </div>
...
const selectedColor = ref(product_.colors[0])
const selectedSize = ref(product_.sizes[2])
</script>
<script>
import {router} from '@inertiajs/vue3'

export default {
    name: "ProductQuickViews",
    props: ['open', 'product'],
    watch: {
        open(old, new_) {
            this.show = true
    ...
    methods: {
        AddToCart(item) {
            ...
            router.post('/cart', {'product': this.product})
        }
    }
}
</script>
